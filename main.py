...
class ARDrumApp:
                ...
                if self.is_calibrated:
                    ...
                    if self.show_hit_messages:
                        ...
                        if cur_time - self.last_l_hit_time < 0.5:
                            cv2.putText(image, "LEFT HAND HIT!", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)
                        if cur_time - self.last_r_hit_time < 0.5:
                            cv2.putText(image, "RIGHT HAND HIT!", (self.frame_width - 350, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)
    ...
    def _draw_drums(self, image, cur_time):
        ...
        for name, pos in self.cached_drum_positions.items():
            ...
            cv2.ellipse(overlay, (pos["cx"], pos["cy"]), (pos["rx"], pos["ry"]), 0, 0, 360, color, -1 if is_hit else 2)
        cv2.addWeighted(overlay, 0.5, image, 0.5, 0, image)

        # Updated UI Help Text to include [J]
        cv2.putText(image, "[D] Drums [C] Coords [H] Occlusion [J] Hit Msg", (10, self.frame_height - 20), 
